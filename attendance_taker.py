...
class Face_Recognizer:
    ...
    def get_face_database(self):
        if os.path.exists("data/features_all.csv"):
            ...
            logging.info("Faces in Database： %d", len(
                self.face_features_known_list))
            return 1
        else:
    ...
    def draw_note(self, img_rd):
        ...
        cv2.putText(img_rd, "Face Recognizer with Deep Learning",
        ...
        if self.get_face_database():
            while stream.isOpened():
                ...
                if (self.current_frame_face_cnt == self.last_frame_face_cnt) and (
                    ...
                    logging.debug(
                        "scene 1:   No face cnt changes in this frame!!!")
                ...
                # 6.2  If cnt of faces changes, 0->1 or 1->0 or ...
                    ...
                    # 6.2.2 / Face cnt increase: 0->1, 0->2, ..., 1->2, ...
                        ...
                        for k in range(len(faces)):
                            logging.debug(
                                "  For face %d in current frame:", k + 1)
                            self.current_frame_face_centroid_list.append(
                                [int(faces[k].left() + faces[k].right()) / 2,
                                 int(faces[k].top() + faces[k].bottom()) / 2])
                            ...
                            for i in range(len(self.face_features_known_list)):
                                ...
                                if str(self.face_features_known_list[i][0]) != '0.0':
                                    ...
                                    logging.debug(
                                        "      with person %d, the e-distance: %f", i + 1, e_distance_tmp)
                            ...
                            if min(self.current_frame_face_X_e_distance_list) < 0.4:
                                ...
                                print(
                                    type(self.face_name_known_list[similar_person_num]))
                                print(nam)
                                self.attendance(nam)
                            else:
                                logging.debug(
                                    "  Face recognition result: Unknown person")
